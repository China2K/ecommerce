...
/*
 * global $, QUnit, window, document, expect, module, test, asyncTest, start,
 * ok, strictEqual, notStrictEqual
 */
...
$(function() {
	...
	test('Data attribute options', function() {
				...
				strictEqual($('#fileupload').fileupload('option', 'url'),
						'http://example.org');
			});

	test('File input initialization', function() {
				var fu = $('#fileupload').fileupload();
				ok(fu.fileupload('option', 'fileInput').length,
	...
	test('Drop zone initialization', function() {
		ok($('#fileupload').fileupload().fileupload('option', 'dropZone').length);
	});

	test('Paste zone initialization', function() {
		ok($('#fileupload').fileupload().fileupload('option', 'pasteZone').length);
	});
	...
	test('Event listeners initialization', function() {
		...
		var eo = {
			originalEvent : {
				...
				}), fileInput = fu.fileupload('option', 'fileInput'), dropZone = fu
				.fileupload('option', 'dropZone'), pasteZone = fu.fileupload(
				'option', 'pasteZone');
	...
	test('destroy', function() {
				...
				}, fu = $('#fileupload').fileupload(options), fileInput = fu
						.fileupload('option', 'fileInput'), dropZone = fu
						.fileupload('option', 'dropZone'), pasteZone = fu
	...
	asyncTest('add', function() {
		...
		$('#fileupload').fileupload({
			add : function(e, data) {
				strictEqual(data.files[0].name, param.files[0].name,
						'Triggers add callback');
			}
		}).fileupload('add', param).fileupload('option', 'add',
				function(e, data) {
	...
	asyncTest('send', function() {
				...
				$('#fileupload').fileupload({
					send : function(e, data) {
						strictEqual(data.files[0].name, 'test',
	...
	asyncTest('fail', function() {
		...
		(fu.data('blueimp-fileupload') || fu.data('fileupload'))._isXHRUpload = function() {
			return true;
		};
	...
	asyncTest('always', function() {
		...
		}, counter = 0, fu = $('#fileupload').fileupload({
					always : function() {
		...
		fu.fileupload('add', param).fileupload('option', 'url', '404')
				.fileupload('add', param);
	});
	...
	asyncTest('start', function() {
				...
				$('#fileupload').fileupload({
							...
							start : function() {
								ok(!active,
										'Triggers start callback before uploads');
								start();
							}
						}).fileupload('send', param);
			});
	...
	asyncTest('stop', function() {
				...
				$('#fileupload').fileupload({
							...
							stop : function() {
								ok(!active,
										'Triggers stop callback after uploads');
								start();
							}
						}).fileupload('send', param);
			});
	...
	test('change', function() {
				var fu = $('#fileupload').fileupload(), fuo = fu
						.data('blueimp-fileupload')
						|| fu.data('fileupload'), fileInput = fu.fileupload(
						'option', 'fileInput');
				expect(2);
				fu.fileupload({
							change : function(e, data) {
								ok(true, 'Triggers change callback');
								strictEqual(data.files.length, 0,
	...
	test('paramName', function() {
				...
				$('#fileupload').fileupload({
					paramName : null,
					send : function(e, data) {
						strictEqual(data.paramName[0], data.fileInput
										.prop('name'),
	...
	test('url', function() {
				...
				$('#fileupload').fileupload({
					url : null,
					send : function(e, data) {
						strictEqual(data.url, $(data.fileInput.prop('form'))
										.prop('action'),
	...
	test('replaceFileInput', function() {
		var fu = $('#fileupload').fileupload(), fuo = fu
				.data('blueimp-fileupload')
				|| fu.data('fileupload'), fileInput = fu.fileupload('option',
				'fileInput'), fileInputElement = fileInput[0];
		expect(2);
		fu.fileupload({
					replaceFileInput : false,
					change : function() {
						strictEqual(fu.fileupload('option', 'fileInput')[0],
								fileInputElement,
		...
		fu.fileupload({
					...
					change : function() {
						notStrictEqual(fu.fileupload('option', 'fileInput')[0],
								fileInputElement,
	...
	asyncTest('forceIframeTransport', function() {
				...
				$('#fileupload').fileupload({
					forceIframeTransport : true,
						...
						strictEqual(data.dataType.substr(0, 6), 'iframe',
	...
	test('singleFileUploads', function() {
		expect(3);
		var fu = $('#fileupload').fileupload(), param = {
			files : [{
						name : '1'
					}, {
						name : '2'
		...
		$('#fileupload').fileupload({
				...
				}).fileupload('add', param).fileupload('option',
				'singleFileUploads', false).fileupload('add', param);
	});
	...
	test('limitMultiFileUploadSize', function() {
		...
		var fu = $('#fileupload').fileupload(), param = {
			files : [{
						...
						name : '4-1',
						size : 45000
					} // New request due to limitMultiFileUploads
			]
		}, param2 = {
			files : [{
						name : '5-1'
	...
	asyncTest('sequentialUploads', function() {
		...
		}, addIndex = 0, sendIndex = 0, loadIndex = 0, fu = $('#fileupload')
				.fileupload({
		...
		asyncTest('multipart', function() {
			...
			var param = {
				...
				always : function(e, data) {
					strictEqual(data.contentType, param.files[0].type,
							'non-multipart upload sets file type as contentType');
					strictEqual(data.headers['Content-Disposition'],
	...
	test('Widget initialization', function() {
				...
				ok(
						$('#fileupload').fileupload('option',
								'downloadTemplate').length,
						'Initialized download template');
			});

	test('Buttonbar event listeners', function() {
		var buttonbar = $('#fileupload .fileupload-buttonbar'), files = [{
					name : 'test'
				}];
		expect(4);
		$('#fileupload').fileupload({
					...
					fail : function(e, data) {
						ok(true,
								'Canceled file upload via global cancel button');
						data.context.remove();
					},
					destroy : function() {
						ok(true,
								'Delete action called via global delete button');
					}
				});
		...
		($('#fileupload').data('blueimp-fileupload') || $('#fileupload')
				.data('fileupload'))._renderDownload(files)
				.appendTo($('#fileupload .files')).show().find('.toggle')
				.click();
	...
	test('disable/enable', function() {
		...
		strictEqual(
				...
				0, 'Disables the buttonbar buttons');
		$('#fileupload').fileupload('enable');
		strictEqual(
				buttonbar.find('input[type=file], button').not(':disabled').length,
				4, 'Enables the buttonbar buttons');
	});
	...
	test('destroy', function() {
				...
				$('#fileupload').fileupload({
					destroy : function(e, data) {
						...
						strictEqual(data.url, 'test',
								'Passes over deletion url parameter');
						strictEqual(data.type, 'DELETE',
								'Passes over deletion request type parameter');
					}
				});
				($('#fileupload').data('blueimp-fileupload') || $('#fileupload')
						.data('fileupload'))._renderDownload([{
							name : 'test',
							deleteUrl : 'test',
							deleteType : 'DELETE'
	...
	test('maxFileSize', function() {
				...
				$('#fileupload').fileupload({
					...
					send : function(e, data) {
						ok(		!$.blueimp.fileupload.prototype.options.send
										.call(this, e, data));
						return false;
					}
				});
	...
	test('acceptFileTypes as HTML5 data attribute', function() {
				...
				$('#fileupload').attr('data-accept-file-types',
						regExp.toString()).fileupload();
				strictEqual($.type($('#fileupload').fileupload('option',
								'acceptFileTypes')), $.type(regExp));
				strictEqual($('#fileupload').fileupload('option',
								'acceptFileTypes').toString(), regExp
								.toString());
			});
