...
(function(factory) {
	...
	if (typeof define === 'function' && define.amd) {
		...
		define(	['jquery', 'load-image', 'load-image-meta', 'load-image-exif',
						'load-image-ios', 'canvas-to-blob',
						'./jquery.fileupload-process'], factory);
	} else {
		// Browser globals:
	...
	$.widget('blueimp.fileupload', $.blueimp.fileupload, {
		...
		processActions : {
			...
			loadImage : function(data, options) {
				...
				var that = this, file = data.files[data.index], dfd = $
						.Deferred();
			...
			resizeImage : function(data, options) {
				...
				var that = this, dfd = $.Deferred(), img = (options.canvas && data.canvas)
						|| data.img, resolve = function(newImg) {
					if (newImg
							&& (newImg.width !== img.width
									|| newImg.height !== img.height || options.forceResize)) {
			...
			saveImageMetaData : function(data, options) {
				...
				var file = data.files[data.index], blob = new Blob([
								...
								// bytes,
								// including the JPEG marker and a minimal JFIF
								// header:
